 ...
 */
class Woo_Checkout_Network_Admin_Settings_Addons
{
    ...
    public function __construct($settings_instance)
    {
        ...
        add_action('wp_ajax_woo_checkout_get_network_api_status_ajax', array($this, 'get_network_api_status_ajax'));

        //Add ajax action to save form data
        add_action('wp_ajax_woo_checkout_network_addon_form_data_ajax', array($this, 'save_network_form_data_ajax'));
    ...
    public function get_network_addon_information($addon_information)
    {
        if ($this->plugin_api_version === 'Paid' && empty($addon_information)) {
            ...
            $addon_information = array(
                ...
                'description' => __("This dashboard will allow you to quickly manage high-level features of this add-on and see global use of API keys", "wp-checkout-vis-fields-woo")
            );
        }
    ...
    public function get_domain_settings()
    {
        ...
        foreach ($domain_list as $domain) {
            ...
            $accordion_settings = array(
                ...
                'title' => __($title . ' (' . $domain_url . ')', "wp-checkout-vis-fields-woo")
            );

            //get_blog_option( int $id, string $option, mixed $default = false )
            $hide_shipping_fields_toggle = get_blog_option($blog_id, 'woo_checkout_hide_shipping_fields_toggle');
            $hide_billing_fields_toggle = get_blog_option($blog_id, 'woo_checkout_hide_billing_fields_toggle');
            $results_returned = get_blog_option($blog_id, 'woo_checkout_results_returned');
            $cron_log = get_blog_option($blog_id, 'woo_checkout_cron_log');

            //Define table data
            $admin_url = get_admin_url($blog_id);   //Get admin url based on domain
            $table_section_array = array(
                array(
                    'title' =>  __("Add-on activation status", "wp-checkout-vis-fields-woo"),
                    'type' => '',
                    'additional_content' => '
                    <div class="additional-content">
                        <a href="' . $admin_url . 'admin.php?page=zamartz-settings&tab=addons&section=' . $this->get_plugin_section_slug() . '">
                        ' . __("[Network activated]", "wp-checkout-vis-fields-woo") . '
                        </a>
                    </div>'
                ),
                array(
                    'title' =>  __("WooCommerce Status", "wp-checkout-vis-fields-woo"),
                    'type' => '',
                    'additional_content' => '
                    <div class="additional-content">
                        <a href="' . $admin_url . 'admin.php?page=wc-status">
                        ' . __("[Site activated]", "wp-checkout-vis-fields-woo") . '
                        </a>
                    </div>'
                ),
                array(
                    'title' =>  __("API Key Status", "wp-checkout-vis-fields-woo"),
                    'type' => '',
                    'additional_content' => '
                    <div class="additional-content">
                ...
                array(
                    'title' =>  __("CRON Log", "wp-checkout-vis-fields-woo"),
                    'tooltip_desc' => __("Enabling this will create a debug log for each time the activation status cron job is run. Turning off will delete the log.", "wp-checkout-vis-fields-woo"),
                    'type' => 'toggle_switch',
                    'option_settings' => array(
                    ...
                    'additional_content' => '
                    <div class="additional-content">
                        ' . __("Created per site", "wp-checkout-vis-fields-woo") . '
                    </div>'
                ),
            );
    ...
    public function save_network_form_data_ajax()
    {
        ...
        foreach ($postArray as $option_name => $data) {
            if (empty($option_name) || strpos($option_name, "woo_checkout") === false || $option_name == 'zamartz_settings_nonce') {
                continue;
            }
